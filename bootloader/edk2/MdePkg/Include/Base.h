...
#define VA_START(Marker, Parameter) (Marker).GrTop = (Marker).VrTop = NULL, (Marker).GrOffs = (Marker).VrOffs = 0, __va_start(&(Marker).Stack, &Parameter, _INT_SIZE_OF(Parameter), __alignof(Parameter), &Parameter)
#define VA_STACK_ARG(Marker, TYPE)  (((Marker).Stack += _INT_SIZE_OF(TYPE) + ((-(INTN) (Marker).Stack) & (sizeof(TYPE) - 1))) - _INT_SIZE_OF(TYPE))
#define VA_ARG(Marker, TYPE)        (*(TYPE *) (((Marker).GrOffs += _INT_SIZE_OF(TYPE)) <= 0 ? &(Marker).GrTop[(Marker).GrOffs - _INT_SIZE_OF(TYPE)] : VA_STACK_ARG(Marker, TYPE)))
