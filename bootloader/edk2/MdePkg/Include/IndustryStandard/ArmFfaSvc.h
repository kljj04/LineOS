...
#define ARM_FFA_MAJOR_VERSION_GET(version) (((version) >> ARM_FFA_MAJOR_VERSION_SHIFT) & ARM_FFA_MAJOR_VERSION_MASK)

#define ARM_FFA_MINOR_VERSION_GET(version) (((version) >> ARM_FFA_MINOR_VERSION_SHIFT) & ARM_FFA_MINOR_VERSION_MASK)

#define ARM_FFA_CREATE_VERSION(major, minor) (((major) << ARM_FFA_MAJOR_VERSION_SHIFT) | ((minor) << ARM_FFA_MINOR_VERSION_SHIFT))

#define ARM_FFA_ABI_COMPATIBLE(version, major, minor) ((BOOLEAN) (((ARM_FFA_MAJOR_VERSION_GET(version)) == major) && ((ARM_FFA_MINOR_VERSION_GET(version)) >= minor)))

#define ARM_FFA_ABI_MINIMUM(version, major, minor) ((BOOLEAN) (((ARM_FFA_MAJOR_VERSION_GET(version)) < major) ? FALSE : (((ARM_FFA_MAJOR_VERSION_GET(version)) > major) ? TRUE : ((ARM_FFA_MINOR_VERSION_GET(version)) >= minor))))
...
#define ARM_FFA_SET_MEM_ATTR_MAKE_PERM_REQUEST(dataperm, codeperm) ((((codeperm) & ARM_FFA_SET_MEM_ATTR_CODE_PERM_MASK) << ARM_FFA_SET_MEM_ATTR_CODE_PERM_SHIFT) | (((dataperm) & ARM_FFA_SET_MEM_ATTR_DATA_PERM_MASK) << ARM_FFA_SET_MEM_ATTR_DATA_PERM_SHIFT))
...
#define GET_SOURCE_PARTITION_ID(PackedId) ((PackedId >> ARM_FFA_SOURCE_EP_SHIFT) & ARM_FFA_PARTITION_ID_MASK)

#define GET_DEST_PARTITION_ID(PackedId) ((PackedId >> ARM_FFA_DEST_EP_SHIFT) & ARM_FFA_PARTITION_ID_MASK)

#define PACK_PARTITION_ID_INFO(SourceId, DestId) (((SourceId & ARM_FFA_PARTITION_ID_MASK) << ARM_FFA_SOURCE_EP_SHIFT) | ((DestId & ARM_FFA_PARTITION_ID_MASK) << ARM_FFA_DEST_EP_SHIFT))

#define ARM_FFA_IS_SECURE_PARTITION_ID(PartId) (((PartId) & (1 << ARM_FFA_PARTITION_ID_TYPE_SHIFT)) != 0)

#define IS_FID_FFA_ERROR(fid) (fid == ARM_FID_FFA_ERROR)
